import { AbstractControl, UntypedFormGroup, ValidatorFn, FormGroup, FormControl } from '@angular/forms';
...
export class GenericValidators {
    ...
    */
    ...
    /**
   * Validar que la contraseña contenga números, letras mayúsculas y minúsculas y un carácter especial
   * @param control valor de control de formulario
   */
    ...
    */
    static onlyNumberValidator(control: AbstractControl): { [key: string]: boolean } | null {
     ...
     /**
    * Validar que todos los caracteres sean solo números decimales
    * @param control valor de control de formulario
    */
    static onlyNumberDecimalValidator(control: AbstractControl): { [key: string]: boolean } | null {
        if (control.value != null && control.value.toString().trim() !== "") {
            // Modificamos la expresión regular para permitir números enteros y decimales
            if (/^[0-9]+(\.[0-9]+)?$/.test(control.value)) {
                return null;
            } else {
                return { 'IsNotNumberDecimal': true };
            }
        } else {
            return null;
        }
      ...
      /**
     * Validar que todos los caracteres sean solo números enteros o decimales, y terminen con el símbolo %
     * @param control valor de control de formulario
     */
    static onlyNumberDecimalWithPercentValidator(control: AbstractControl): { [key: string]: boolean } | null {
        if (control.value != null && control.value.toString().trim() !== "") {
            // Expresión regular para permitir números enteros, decimales, y que terminen con %
            if (/^[0-9]+(\.[0-9]+)?%$/.test(control.value)) {
                return null;
            } else {
                return { 'IsNotNumberDecimalWithPercent': true };
            }
        } else {
            return null;
        }
    ...
    /**
    * Validar que todos los caracteres sean solo números enteros
    * @param control valor de control de formulario
    */
    ...
    * @param control valor de control de formulario
    */
     static noSpacesValidator(control: AbstractControl): { [key: string]: boolean } | null {
        if (control.value != null && control.value.toString().includes(' ')) {
            return { 'hasSpaces': true }; // Inválido si contiene espacios
        } else {
            return null; // Válido
        }
    }
    ...
    */
    static onlyTextAndSpaceValidator(control: AbstractControl): { [key: string]: boolean } | null {
        if (control.value != null && control.value.toString().trim() != "") {
            if (/^[A-Za-zñÑ ]+$/.test(control.value)) return null;
    ...
    */
    ...
    /**
     * Aplica la lógica para convertir el valor del campo a mayúsculas
     * @param fieldName Nombre del campo en el formulario
     */
     applyUpperCaseLogic(fieldName: string): void {
        ...
        if (control instanceof FormControl) {
            ...
            console.error(`Campo '${fieldName}' no encontrado en el formulario o no es un control de formulario.`);
        }
    }
